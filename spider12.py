...
logging.basicConfig(
    filename="spider12.log",
    filemode="a+",
    format="%(asctime)s-%(process)d-%(levelname)s-%(message)s"
)

class KhabarSpider(scrapy.Spider):
    name = "Khabar_spider"
    allowed_domains = ['irinn.ir']

    if rss_reader('https://www.irinn.ir/fa/rss/allnews') == 'Success':
    ...
    def parse(self, response):
        ...
        dic["url"] = "https://irinn.ir" + news_url

        sections = response.xpath('//div[@class="news_path col-xs-36 col-sm-18"]/a/text()').getall()
        dic["article_section"] = sections

        summary = response.xpath('//div[@class="subtitle"]/text()').get()
        dic["summary"] = summary

        date = response.xpath('//div[@class="news_nav news_pdate_c col-xs-36 col-sm-15"]/text()').get()
        date_list = date.split(' ')
        # print(date_list)
        timelist = date_list[6].split(':')
        ...
        code = response.xpath('//div[@class="news_nav news_id_c col-xs-36 col-sm-10"]/text()').get()
        dic["code"] = code

        tags = response.xpath('//div[@class="tags_title"]/a/text()').getall()
        dic["tags"] = tags

        text_list = response.xpath('//div[@class="body col-xs-36"]/div[@dir="rtl"]/text()').getall()
        text = ""
        for t in text_list:
            text += t
        dic["text"] = text
